---
title: Mosslight
year: 2024
format: Visual experiment
category: AI-assisted
status: In development
featured: false
logline: A forest remembers the people who pass through it.
heroImage: /manus-storage/film-card-still_16d5ff5a.jpg
heroAlt: A mossy forest and distant mountain range at twilight.
posterImage: /manus-storage/film-card-still_16d5ff5a.jpg
trailerUrl: null
credits:
  - role: Concept and direction
    name: Little Wild Films
  - role: Generative image research
    name: Little Wild Films
gallery:
  - src: /manus-storage/film-card-still_16d5ff5a.jpg
    alt: A mossy forest study at blue hour.
    caption: First world-building pass.
relatedPosts:
  - teaching-a-machine-to-notice
seoDescription: Mosslight is an AI-assisted visual experiment by Little Wild Films.
---

## About the film

Mosslight began as a question: can a place hold a memory without turning it into a ghost story?

The project uses generative tools as a sketchbook for a forest that changes with every person who enters it.

## The experiment

Rather than asking a model for a finished image, we are using it to discover the edges of a world — then bringing those discoveries back into a more deliberate filmmaking process.
