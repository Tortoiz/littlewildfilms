---
title: After the Rain
year: 2026
format: Live-action short
category: Live action
status: Coming soon
featured: false
logline: A city becomes strange when the last light goes out.
heroImage: /manus-storage/hero-film-still_04dfca7c.jpg
heroAlt: A moody cinematic landscape under a deep evening sky.
posterImage: /manus-storage/hero-film-still_04dfca7c.jpg
trailerUrl: null
credits:
  - role: Directed by
    name: Little Wild Films
  - role: Cinematography
    name: In-house development
gallery:
  - src: /manus-storage/hero-film-still_04dfca7c.jpg
    alt: A landscape holding the last light after rain.
    caption: Location and weather study.
  - src: /manus-storage/film-card-still_16d5ff5a.jpg
    alt: A rain-darkened valley under a clearing sky.
    caption: Atmosphere reference.
  - src: /manus-storage/hero-film-still_04dfca7c.jpg
    alt: Distant hills emerging from the weather.
    caption: Transition frame.
relatedPosts:
  - the-weather-between-scenes
seoDescription: After the Rain is a live-action short coming soon from Little Wild Films.
---

## About the film

The rain stops. The power goes out. For a few minutes, the city has to look at itself without the usual glow.

After the Rain is a live-action study in atmosphere, memory, and the things that become visible when the familiar systems go quiet.

## Working note

This film is being developed as a compact, location-led piece: a small crew, practical light, and a camera willing to wait.
